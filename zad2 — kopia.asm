;requires input in form of single digit and short text seperated with one space
...
args: ;reads and splits arguments into scale and text to draw
	...
	read_scale:
		...
		je		zero
		inc 	si 		;there are still characters remaining
		dec 	cl 		;we move past the space seperating digit and text to draw
		cmp 	cl, 0
		je 		zero
		cmp 	cl, 41
		jle 	read_txt
	...
	zero:
		...
		mov 	es, ax
		mov 	di, offset text_len
		mov 	byte ptr es:[di], 0 ;set text_len - number of bytes left
		ret
	not_digit:
